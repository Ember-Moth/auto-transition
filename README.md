...
```tsx
import { AutoTransition, effects, preset } from "auto-transition";
...
```ts
import { effects } from "auto-transition";
...
```ts
import { fade, scale, blur, translate, flip } from "auto-transition/effects";
```
